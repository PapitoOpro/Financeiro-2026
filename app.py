        ...
        with st.form("form_caixa", clear_on_submit=True):
            ...
            if st.form_submit_button("Lançar no Caixa", use_container_width=True):
                if not desc_r or val_r <= 0:
                    ...
                    conn.execute("INSERT INTO transacoes (descricao, valor, data_vencimento, conta_id, categoria_id, tipo_fluxo) VALUES (?,?,?,?,?,'CAIXA')", 
            ...
            for _, row in df_caixa.iterrows():
                ...
                with c5:
                    if st.button("🗑️", key=f"del_c_{row['id']}", help="Excluir lançamento"):
                        ...
                        st.rerun() # Adicione isso para a tela atualizar e o registro sumir na hora!
                        conn.commit()
                        st.rerun()
                                ...
                                with c3:
                                    with st.popover("✏️"):
                                        ...
                                        if st.button("Salvar Correção", key=f"save_{r['id']}"):
                                            ...
                                            conn.execute("UPDATE transacoes SET descricao=?, valor=? WHERE id=?", (n_desc, -n_val, r['id']))
                                            conn.commit()
                                            st.rerun()

                                # BOTÃO 🗑️ DELETAR
                                if c4.button("🗑️", key=f"del_t3_{r['id']}"):
                                    conn.execute("DELETE FROM transacoes WHERE id=?", (r['id'],))
                                    conn.commit()
                                    st.rerun()
...
# =======================
    ...
    with c1:
        ...
        with st.form("form_novo_banco", clear_on_submit=True):
            ...
            if col_btn.form_submit_button("Adicionar", use_container_width=True):
                if n_banco.strip():
                    conn.execute("INSERT INTO contas (nome) VALUES (?)", (n_banco.strip(),))
                    conn.commit()
                    st.rerun()
                else:
        ...
        for _, r in df_contas.iterrows():
            ...
            with col_edit:
                with st.popover("✏️"):
                    ...
                    if st.button("Salvar", key=f"save_c_{r['id']}", use_container_width=True):
                        if novo_nome.strip():
                            conn.execute("UPDATE contas SET nome=? WHERE id=?", (novo_nome.strip(), r['id']))
                            conn.commit()
                            st.rerun()
            ...
            with col_del:
                if st.button("🗑️", key=f"del_c_{r['id']}", help="Excluir Conta"):
                    conn.execute("DELETE FROM contas WHERE id=?", (r['id'],))
                    conn.commit()
                    st.rerun()
    ...
    with c2:
        ...
        with st.form("form_nova_cat", clear_on_submit=True):
            ...
            if col_btn.form_submit_button("Adicionar", use_container_width=True):
                if n_cat.strip():
                    conn.execute("INSERT INTO categorias (nome) VALUES (?)", (n_cat.strip(),))
                    conn.commit()
                    st.rerun()
                else:
                    st.error("Digite um nome!")
        ...
        for _, r in df_cats.iterrows():
                ...
                with st.popover("✏️"):
                    ...
                    if st.button("Salvar", key=f"save_cat_{r['id']}", use_container_width=True):
                        if novo_nome_cat.strip():
                            conn.execute("UPDATE categorias SET nome=? WHERE id=?", (novo_nome_cat.strip(), r['id']))
                            conn.commit()
                            st.rerun()
            ...
            with col_del:
                if st.button("🗑️", key=f"del_cat_{r['id']}", help="Excluir Categoria"):
                    conn.execute("DELETE FROM categorias WHERE id=?", (r['id'],))
                    conn.commit()
                    st.rerun()
