...
@st.cache_data(ttl=30)
def _fetch_subcategorias(user_id):
    return db.buscar(
        """
...
@st.cache_data(ttl=30)
def _fetch_extrato(user_id, data_inicio, data_fim):
    return db.buscar(
        """
...
def _invalidar_cache():
    """Limpa o cache de leitura após qualquer mutação no banco."""
    st.cache_data.clear()
...
class CaixaManager:
    ...
    def renderizar():
        ...
        ano_sel = col_ano.number_input(
            "Ano:", min_value=ano_atual - 1, max_value=ano_atual + 2, value=ano_atual,
        )
        ...
        # ── 3. Carrega dados (com cache) ──────────────────────────────────────
        user_id = db.get_user_id()
        df_contas = _fetch_contas(user_id)
        df_cats = _fetch_categorias(user_id)
        ...
        if not df_caixa.empty:
            ...
            compensados = (
                df_caixa["compensado"].sum() if "compensado" in df_caixa.columns else 0
            )
        ...
        col_form, _, col_extrato = st.columns([COL_FORM, COL_ESPACO, COL_EXTRATO])
    ...
    @staticmethod
    def _card_html(label, valor_html, cor_borda=None, bg="#f1f2f6", cor_texto="#333333"):
        borda = f"border-left:5px solid {cor_borda};" if cor_borda else ""
        return (
            f'<div style="background:{bg}; color:{cor_texto}; padding:15px; '
            f"border-radius:10px; flex:1; {borda} min-width:120px;\">"
            f"<small>{label}</small><br>{valor_html}</div>"
        )
    ...
    def _renderizar_cards(ent, sai, bal, saldo_anterior=0.0, compensados=0, pendentes=0):
        ...
        html = (
            ...
            + card(
                ...
                cor_borda=CORES["entrada"],
            )
            + card(
                "Saídas",
                f'<strong style="font-size:20px; color:{CORES["negativo"]};">-{moeda(sai)}</strong>',
                cor_borda=CORES["saida"],
            )
            + card(
                "Balanço Final",
                f'<strong style="font-size:22px;">{moeda(bal)}</strong>',
                bg=bg_bal,
                cor_texto="#ffffff",
            )
            + card(
                "Compensação",
                f'<strong style="font-size:14px; color:#2ecc71;">{compensados}</strong>'
                f'<strong style="font-size:14px; color:{cor_pend};"> | {pendentes}</strong>',
                cor_borda=cor_pend,
            )
            + "</div>"
        )
            ...
            if st.form_submit_button("Lançar no Caixa", width="stretch"):
                if not desc_r or val_r <= 0:
                    ...
                    db.executar(
                        ...
                        "(descricao, valor, data_vencimento, conta_id, categoria_id, "
                        "subcategoria_id, tipo_fluxo, compensado, data_compensacao, user_id) "
                        "VALUES (?,?,?,?,?,?,'CAIXA',?,?,?)",
                        (desc_r, valor_final, data_pg, cid, cat_id_sel,
    ...
    def _renderizar_extrato(df_caixa, df_contas, df_cats):
        ...
        filtro_comp = st.segmented_control(
            "Filtrar:",
            ["Todos", "Pendentes", "Compensados"],
            default="Todos",
            key="filtro_compensacao",
        )
        ...
        ids_pendentes_normais = (
            df_normais[df_normais["compensado"] == False]["id"].tolist()
            if not df_normais.empty and "compensado" in df_normais.columns
            else []
        )
        selected_comp_ids = [
            ...
            with col_btn:
                if st.button(
                    ...
                    for sid in selected_comp_ids:
                        db.executar(
                            "UPDATE transacoes SET compensado=TRUE, data_compensacao=? "
            ...
            for fatura_id_val, grp in df_faturas.groupby("fatura_id"):
                ...
                with c_pagar:
                    if not is_compensado:
                        if st.button(
                            "Pagar Fatura",
                            key=f"pagar_fat_{fatura_id_val}",
                            width="stretch",
                            icon=":material/check:",
                        ):
                        ...
                        if st.button(
                            "Reabrir Fatura",
                            key=f"reabrir_fat_{fatura_id_val}",
                            width="stretch",
                            icon=":material/undo:",
                        ):
                ...
                if st.session_state.get(f"confirm_del_fat_{fatura_id_val}", False):
                    ...
                    if cc1.button(
                        "Sim, excluir",
                        key=f"yes_del_fat_{fatura_id_val}",
                        icon=":material/check:",
                    ):
                        ...
                        st.rerun()
                    if cc2.button(
                        "Não", key=f"no_del_fat_{fatura_id_val}", icon=":material/close:"
                    ):
                        st.session_state.pop(f"confirm_del_fat_{fatura_id_val}", None)
                        st.rerun()

                df_itens_fat = (
                    df_all_itens[df_all_itens["fatura_id"] == fatura_id_val]
                    if not df_all_itens.empty
                    else pd.DataFrame()
                )
                    ...
                    if df_itens_fat.empty:
                        ...
                        for _, item in df_itens_fat.iterrows():
                            ...
                            if item.get("parcela_atual") and item.get("parcela_total"):
                                parc_label = (
                                    f" ({int(item['parcela_atual']):02d}"
                                    f"/{int(item['parcela_total']):02d})"
                                )

                            if not is_editing_item:
                                ci1, ci2, ci3, ci4, ci5 = st.columns([1.3, 3.5, 2.0, 0.5, 0.5])
                                data_compra_str = (
                                    pd.to_datetime(item["data_compra"]).strftime("%d/%m/%Y")
                                    if item.get("data_compra") and not pd.isnull(item["data_compra"])
                                    else ""
                                )
                                ...
                                ci3.markdown(
                                    ...
                                    f"{moeda(abs(item['valor']))}</div>",
                                    unsafe_allow_html=True,
                                )
                                with ci4:
                                    if st.button(
                                        "​",
                                        key=f"edit_fi_{item_id}",
                                        help="Editar item",
                                        icon=":material/edit:",
                                    ):
                                ...
                                if st.session_state.get(f"confirm_del_fi_{item_id}", False):
                                    ...
                                    if cd1.button(
                                        "Sim", key=f"yes_del_fi_{item_id}", icon=":material/check:"
                                    ):
                                        db.executar(
                                            "DELETE FROM itens_fatura WHERE id=? AND user_id=?",
                                            (item_id, user_id),
                                        )
                                        ...
                                        st.rerun()
                                    if cd2.button(
                                        "Não", key=f"no_del_fi_{item_id}", icon=":material/close:"
                                    ):
                                        st.session_state.pop(f"confirm_del_fi_{item_id}", None)
                                        st.rerun()
                            else:
                                with st.form(f"form_edit_fi_{item_id}"):
                                    d_desc = st.text_input(
                                        ...
                                        key=f"efi_desc_{item_id}",
                                    )
                                    d_val = st.number_input(
                                        "Valor (R$)",
                                        min_value=0.0,
                                        value=float(abs(item["valor"])),
                                        key=f"efi_val_{item_id}",
                                    )
                                    cat_op = df_cats["nome"].tolist() if not df_cats.empty else []
                                    try:
                                        default_cat_idx = (
                                            cat_op.index(item.get("categoria", ""))
                                            if item.get("categoria", "") in cat_op
                                            else 0
                                        )
                                    except Exception:
                                        default_cat_idx = 0
                                    ...
                                    if save_clicked:
                                        try:
                                            ...
                                            db.executar(
                                                "UPDATE itens_fatura SET descricao=?, valor=?, "
                                                "categoria_id=? WHERE id=? AND user_id=?",
                ...
                st.markdown(
                    "<hr style='margin:5px 0 10px 0; border-top:1px solid #e0e0e0;'>",
                    unsafe_allow_html=True,
                )
        ...
        if not df_normais.empty:
            ...
            for _, row in df_normais.iterrows():
                ...
                if not is_editing:
                    ...
                    badge_comp = (
                        "<span style='background:#2ecc71; color:gray; padding:1px 6px; "
                        "border-radius:4px; font-size:10px;'>OK</span> "
                        if is_compensado
                    ...
                    c3.markdown(
                        f"<div style='text-align:right; color:{cor}; font-weight:bold;'>"
                        f"{moeda(row['valor'])}</div>",
                        unsafe_allow_html=True,
                    )
                    ...
                    if st.session_state.get(f"confirm_del_caixa_{rid}", False):
                        ...
                        cc1, cc2 = st.columns(2)
                        if cc1.button(
                            "Sim", key=f"yes_del_{rid}", icon=":material/check:"
                        ):
                            db.executar(
                            ...
                            st.rerun()
                        if cc2.button(
                            "Não", key=f"no_del_{rid}", icon=":material/close:"
                        ):
                            st.session_state.pop(f"confirm_del_caixa_{rid}", None)
                            st.rerun()
                else:
                    ...
                    if btn1.button(
                        "Salvar", key=f"ec_save_{rid}", width="stretch", icon=":material/save:"
                    ):
                        ...
                        data_comp = (
                            datetime.now().date()
                            if n_compensado and not is_compensado
                            else (row.get("data_compensacao") if n_compensado else None)
                        )
                        db.executar(
                            "UPDATE transacoes SET descricao=?, valor=?, data_vencimento=?, "
                            "conta_id=?, categoria_id=?, compensado=?, data_compensacao=? "
                            "WHERE id=? AND user_id=?",
                ...
                if not is_editing:
                    st.markdown(
                        "<hr style='margin:0px 0px 10px 0px; padding:0; "
                ...
                if fatura_id_val and not pd.isna(fatura_id_val):
                    with st.expander("Ver itens da fatura", expanded=False):
